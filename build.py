...
def main():
    ...
    if windows:
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/shopremote2')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv shopremote2-%s-0-x86_64.pkg.tar.zst shopremote2-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./shopremote2.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/shopremote2-%s-0.x86_64.rpm ./shopremote2-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall shopremote2.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/shopremote2-%s-0.x86_64.rpm ./shopremote2-%s-suse.rpm' % (
                version, version))
        # yum localinstall shopremote2.rpm
    else:
        if flutter:
