    ...
    loop {
        ...
        if !sids.contains(&session_id) || !is_share_rdp() {
            ...
            if session_id != current_active_session {
                ...
                // https://github.com/ccaplee/shopremote2/discussions/10039
...
fn get_after_install(
    ...
    // https://github.com/ccaplee/shopremote2/commit/f4bdfb6936ae4804fc8ab1cf560db192622ad01a
...
copy /Y \"{tmp_path}\\Uninstall {app_name}.lnk\" \"{start_menu}\\\"
    ...
    // Note: without if exist, the bat may exit in advance on some Windows7 https://github.com/ccaplee/shopremote2/issues/895
    let dels = format!(
        "
...
fn run_cmds(cmds: String, show: bool, tip: &str) -> ResultType<()> {
    ...
    // https://github.com/ccaplee/shopremote2/issues/6786#issuecomment-1879655410
...
reg add {subkey} /f /v EstimatedSize /t REG_DWORD /d {size}
    ...
    // 1. There may be some other processes like `shopremote2 --connect` are running.
    ...
    for entry in entries {
        if let Ok(entry) = entry {
            ...
            if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                // Match files like shopremote2-*.msi or shopremote2-*.exe
                if file_name.starts_with("shopremote2-")
...
pub fn try_kill_shopremote2_main_window_process() -> ResultType<()> {
    // Kill shopremote2.exe without extra arg, should only be called by --server
    ...
    bail!("failed to find shopremote2 main window process");
}
...
pub fn send_raw_data_to_printer(printer_name: Option<String>, data: Vec<u8>) -> ResultType<()> {
    ...
    unsafe {
        ...
        if res != 0 {
            bail!("Failed to send data to the printer, see logs in C:\\Windows\\temp\\test_shopremote2.log for more details.");
        } else {
