...
import 'package:shopremote2/desktop/widgets/refresh_wrapper.dart';
import 'package:shopremote2/desktop/widgets/tabbar_widget.dart';
import 'package:shopremote2/main.dart';
import 'package:shopremote2/models/peer_model.dart';
import 'package:shopremote2/models/peer_tab_model.dart';
...
import 'package:shopremote2/native/win32.dart'
    if (dart.library.html) 'package:shopremote2/web/win32.dart';
import 'package:shopremote2/native/common.dart'
    if (dart.library.html) 'package:shopremote2/web/common.dart';
import 'package:shopremote2/utils/http_service.dart' as http;
...
void msgBox(SessionID sessionId, String type, String title, String text,
  ...
  submit() {
    ...
    if (onSubmit != null) {
      ...
      // https://github.com/ccaplee/shopremote2/blob/5e9a31340b899822090a3731769ae79c6bf5f3e5/src/ui/common.tis#L263
  ...
  if (lpos == null) {
    ...
    switch (type) {
      case WindowType.Main:
        ...
        // See https://github.com/ccaplee/shopremote2/blob/9b9276e7524523d7f667fefcd0694d981443df0e/flutter/macos/Runner/Base.lproj/MainMenu.xib#L333
      ...
      default:
        ...
        // https://github.com/ccaplee/shopremote2/blob/317639169359936f7f9f85ef445ec9774218772d/flutter/lib/utils/multi_window_manager.dart#L163
        break;
    }
    return true;
  }
      ...
      if (lpos.isMaximized == true) {
        ...
        if (isWindows) {
          if (storeSize) {
            ...
            // See known issue in https://github.com/ccaplee/shopremote2/pull/9840
...
List<String>? urlLinkToCmdArgs(Uri uri) {
  ...
  if (uri.authority.isEmpty &&
    ...
    // shopremote2://<connect-id>
    // shopremote2://<connect-id>/r
    // shopremote2://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
...
class ServerConfig {
  ...
  /// decode from shared string (from user shared or shopremote2-server generated)
  /// also see [encode]
...
const kMacOSPermChannel = MethodChannel("org.shopremote2.shopremote2/host");
...
setNewConnectWindowFrame(int windowId, String peerId, int preSessionCount,
  ...
  if (screenRect == null) {
    ...
    // https://github.com/ccaplee/shopremote2/discussions/8825
...
Widget loadPowered(BuildContext context) {
  ...
  return MouseRegion(
    ...
    child: GestureDetector(
      onTap: () {
        launchUrl(Uri.parse('https://ai.ilv.co.kr'));
      },
      child: Opacity(
          opacity: 0.5,
          child: Text(
      ...
      children: [
        Align(
          ...
          style: TextStyle(
            color: Colors.red,
            fontSize:
                18, // https://github.com/ccaplee/shopremote2-server/issues/261
            fontWeight: FontWeight.bold,
          ),
...
// https://github.com/ccaplee/shopremote2/blob/ce1dac3b8613596b4d8ae981275f9335489eb935/flutter/lib/desktop/widgets/tabbar_widget.dart#L385
// https://github.com/ccaplee/shopremote2/blob/ce1dac3b8613596b4d8ae981275f9335489eb935/flutter/lib/desktop/widgets/tabbar_widget.dart#L399
//
// @platforms macos
...
String decode_http_response(http.Response resp) {
  try {
    // https://github.com/ccaplee/shopremote2-server/discussions/758
